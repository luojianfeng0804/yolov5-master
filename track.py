...
from yolov5.utils.general import check_img_size, non_max_suppression, scale_coords, check_imshow, xyxy2xywh,increment_path,calculate_IoU,delimg
            ...
            if det is not None and len(det):
                ...
                for qc in output_qc:
                    for tooth in output_tooth:
                        if(calculate_IoU(qc[0:4],tooth[0:4])>0.5 ):
                            print('qc=',qc)
...
def yolo_predict(fpath):
    ...
    with torch.no_grad():
        ...
        if len(os.listdir(str(save_dir / 'labels'))) == 0:
            print(str(save_dir / 'labels'))
            return img_path
        ...
        for line in lines:
            ...
            if output[0] in frame:
                idex = int(output[0])
                im0 = im0_sum[idex]
                id, name, box=output[1], output[2],output[3:]
                bboxes = list(map(int, box))
                label = f'{id} {name}'
                color = compute_color_for_id(int(id))
                #im0=cv2.imread(os.path.join(img_path,im))
                plot_one_box(bboxes, im0, label=None, color=color, line_thickness=2)
