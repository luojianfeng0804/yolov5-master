...
def compute_IOU(rec1, rec2):
    ...
    # 两矩形有相交区域的情况
        ...
        print('IoU=', S_cross / (S1))
        return S_cross / (S1)

def delimg(txt_file):
    #txt_file = 'D:/BaiduNetdiskDownload/Week5_yolov5-deepsort/Yolov5_DeepSort_Pytorch-master/inference/output/exp26/labels/1_None.txt'
    f = open(txt_file)
    lines = f.readlines()
    txt_tuple = []
    id = []
    for line in lines:
        ...
        if box[1] not in id:
            id.append(box[1])
        txt_tuple.append(box)
    print(txt_tuple)
    print('id=', id)
    len_txt = len(txt_tuple)
    print(len_txt)
    frame_sum = int(txt_tuple[len_txt - 1][0])
    anx = {}

    for t in txt_tuple:
        frame = t[0]
        if frame not in anx.keys():
            anx[frame] = []
        anx[frame].append(t[1])
    print(anx)
    ...
    for i, name in enumerate(id):
        ...
        max_f = ''
        for frame_idx in anx:
            if name in anx[frame_idx]:
                l = len(anx[frame_idx])
                if (max < l):
                    max = l
                    #if max_f == '':
                    max_f = frame_idx
        if max_f not in max_frame:
            max_frame.append(max_f)
        id_sum.append(anx[max_f])
    print(id_sum)
    print(max_frame)
    return max_frame
...
def detooth(outputs):
        ...
        for qc in output_qc:
            for tooth in output_tooth:
                ...
                if (compute_IOU(qc[0:4], tooth[0:4]) > 0.5 or calculate_IoU(qc[0:4], tooth[0:4])):
                    print('qc=', qc)
                    output_final.append(qc)
                    continue
