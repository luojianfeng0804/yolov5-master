...
def imagededep(path):
    #path = r'C:/Users/123/Desktop/results3'
...
def save_img(video_path):
    ...
    while rval:   #循环读取视频帧
            ...
            if c==1:
                i=i+1
                cv2.imwrite(pic_path + str(i).zfill(5) + '.jpg', frame)
            if c % temp == 0:
                    i= i+1
                    cv2.imwrite(pic_path + str(i).zfill(5) + '.jpg', frame)#str(n).zfill(5)设置保存图片文件名格式（5位）00001
            if (total-c) < temp:
                break
